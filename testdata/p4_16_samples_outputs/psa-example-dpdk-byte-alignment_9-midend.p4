...
control ingress(inout headers hdr, inout metadata_t user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @hidden action psaexampledpdkbytealignment_9l69() {
        color_in_0 = PSA_MeterColor_t.RED;
    }
    @hidden table tbl_psaexampledpdkbytealignment_9l69 {
        actions = {
            psaexampledpdkbytealignment_9l69();
        }
        const default_action = psaexampledpdkbytealignment_9l69();
    }
    @hidden table tbl_psaexampledpdkbytealignment_9l100 {
        actions = {
            psaexampledpdkbytealignment_9l100();
        }
        const default_action = psaexampledpdkbytealignment_9l100();
    }
    ...
    apply {
        tbl_psaexampledpdkbytealignment_9l69.apply();
        if (user_meta.port_out == 32w1) {
            tbl_0.apply();
            tbl_psaexampledpdkbytealignment_9l100.apply();
            tbl_test.apply();
        }
    }
}
...
control IngressDeparserImpl(packet_out packet, out empty_metadata_t clone_i2e_meta, out empty_metadata_t resubmit_meta, out empty_metadata_t normal_meta, inout headers hdr, in metadata_t meta, in psa_ingress_output_metadata_t istd) {
    @hidden action psaexampledpdkbytealignment_9l124() {
        hdr.ipv4.hdrChecksum = 16w4;
        packet.emit<ethernet_t>(hdr.ethernet);
        packet.emit<ipv4_t>(hdr.ipv4);
    }
    @hidden table tbl_psaexampledpdkbytealignment_9l124 {
        actions = {
            psaexampledpdkbytealignment_9l124();
        }
        const default_action = psaexampledpdkbytealignment_9l124();
    }
    apply {
        tbl_psaexampledpdkbytealignment_9l124.apply();
    }
}
